...
export default class extends Component {
  ...
  onScrollEnd = e => {
    ...
    this.updateIndex(e.nativeEvent.contentOffset, this.state.dir, () => {
      ...
      if (this.props.onScrollEndCallback) {
        console.log('this.props',this.props);
        console.log('this.fullState',this.fullState());
        if(this.props.children) {
          let current = this.state.index % this.props.children.length;
          this.props.onScrollEndCallback({index: current, key: this.props.children[current].key});
        }
      }
    })
  }
  ...
  scrollBy = (index, animated = true, _default = true) => {
    if (this.internals.isScrolling || this.state.total < 2) return
    const state = this.state
    // const diff = (this.props.loop ? 1 : 0) + index + this.state.index
    // console.log('diff diff diff', this.state.index);
    const diff = (_default) ? ((this.props.loop ? 1 : 0) + index + this.state.index) : index;
    let x = 0
    let y = 0
  ...
  renderScrollView = pages => {
    ...
    return (
      <QuantaViewPager navigator={this.props.navigator} parentCallback={this.props.parentCallback} ref='QuantaViewPagerRef'>
